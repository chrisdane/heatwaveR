...
myrunscript_fname <- "~/scripts/r/heatwaveR/composite_heatwaveR.r"
#myrunscript_fname <- "~/scripts/r/heatwaveR/composite_cdo_heatwaveR.r" # too slow

if (F) { # oisst
    prefix <- "oisst"
    #replace_string <- list(string="    location_inds <- ", between_lines=c(24, 65))
    replace_string <- list(string="    location_inds <- location_inds$ind[", between_lines=c(24, 65))
    stop("update replace_string")
    ...
    #prefix <- "awi-esm-1-1-lr_kh800_piControl_tos"
    #prefix <- "awi-esm-1-1-lr_kh800_piControl_tauuo"
    #prefix <- "awi-esm-1-1-lr_kh800_piControl_tauvo"
    #prefix <- "awi-esm-1-1-lr_kh800_piControl_mlotst"
    #prefix <- "awi-esm-1-1-lr_kh800_piControl_omldamax"
    prefix <- "awi-esm-1-1-lr_kh800_piControl_sic"
    #prefix <- "awi-esm-1-1-lr_kh800_historical2"
    replace_string <- list(string="    select_inds_files <- select_inds_files[", between_lines=c(34, 36))
    start <- 1; end <- 82
    njobs_wanted <- 82
...
if (!exists("replace_by")) {
    ...
    replace_by <- matrix(replace_by, ncol=2)
    if (ntot > 1) replace_by[,2] <- c(replace_by[2:njobs_wanted,2]-1, end)
